...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_RELEASE_GPIO:
		xiaomi_santoni_gf_cleanup(gf_dev);
		break;
	...
	case GF_IOC_SETSPEED:
		...
		break;
	case GF_IOC_RESET:
		xiaomi_santoni_gf_hw_reset(gf_dev, 3);
		break;
	case GF_IOC_COOLBOOT:
		xiaomi_santoni_gf_power_off(gf_dev);
		mdelay(5);
		xiaomi_santoni_gf_power_on(gf_dev);
		break;
	case GF_IOC_SENDKEY:
		if (copy_from_user
...
static int driver_init_partial(struct gf_dev *gf_dev)
{
	...
	if (xiaomi_santoni_gf_parse_dts(gf_dev))
		goto error;

	gf_dev->irq = xiaomi_santoni_gf_irq_num(gf_dev);
	...
	xiaomi_santoni_gf_hw_reset(gf_dev, 360);
